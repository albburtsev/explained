...
description: See why analytics needs its own platform, how ingestion, storage, compute, and orchestration divide the work, and where Iceberg, Trino, Spark, and Airflow sit.
tags:
  - data-engineering
  - data-platform
  - architecture
...
**Compute engines.** The components that read the storage layer, split the work across many machines, and return rows or write new tables. Some engines are built to answer SQL questions interactively. Others are built for long jobs that transform large amounts of data. In this architecture the engines are separate from the storage: the data is not "inside" an engine, and more than one engine can read the same tables.
...
These four are a division of labour, not a strict sequence. Data can loop back: one engine's output becomes a new table that another job reads.
...
| --- | --- | --- |
| Ingestion | Not covered in this course | Exports, change streams, or event pipelines that bring source data into the platform |
...
| Compute: interactive queries | Trino | A distributed SQL engine that queries large datasets across sources it does not own |
| Compute: data processing | Apache Spark | A distributed engine for long jobs that transform data and build tables |
...
Ingestion has no tool in this course. The course treats source data as something that arrives in object storage, and starts where it lands.
...
Spark sits in the same compute job as Trino, and the two are easy to confuse. Both read the same tables, and both accept SQL. Trino is built for questions that a person or a dashboard is waiting on. Spark is built for long jobs that produce data for later queries. The Spark lesson compares them in detail.

Airflow schedules and monitors work; it does not move or transform the data itself. A task tells some other system to do the work. Its model is batch-oriented — runs with a beginning and an end — which is why continuous event streams are handled by other systems.

## Follow one question through the platform

Take the delivery-time question from the opening and trace it.

1. Ingestion jobs copy order and shipment data out of the services' databases and land it as files in object storage. The services themselves only serve their own traffic.
2. A Spark job reads those files and writes them into tables. Iceberg gives those tables a schema, so a column can be added later without rewriting history, and readers never see a half-finished write.
3. Further Spark jobs clean and join the raw tables into ones shaped for reporting.
...
- **The boundaries overlap.** Trino can write tables, and Spark can answer ad hoc SQL; some orchestrators run the transformation themselves. Vendors merge boxes deliberately.
...
3. Trino and Spark both accept SQL. Which one would you expect behind a dashboard, and which behind a nightly job that rebuilds a large table?
...
Expected answers: 1. A transaction reads and writes whole rows, so splitting a row across per-column storage makes the common operation expensive. 2. Orchestration — the scheduling, retry, and visibility layer. 3. Trino behind the dashboard, because it is built for interactive questions; Spark for the nightly job, because it is built for long processing that must survive failures. 4. Data in a storage layer it can read, and table metadata describing it. 5. It skips a separate table format and separate compute engines; that is fine as long as the volume, freshness, and number of sources stay small.
...
The next lesson starts with storage: how Apache Iceberg turns files in object storage into tables.
